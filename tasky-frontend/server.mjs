...
let apiInternalUrl = process.env.API_INTERNAL_URL || 'http://tasky-backend:9000'

if (!apiInternalUrl.startsWith('http://') && !apiInternalUrl.startsWith('https://')) {
  ...
  replyOptions: {
    rewriteRequestHeaders: (originalReq, headers) => {
      try {
        ...
        app.log.error({ error, apiInternalUrl }, 'Error parsing API_INTERNAL_URL')
        return headers
      }
    },
    onError: (reply, error) => {
